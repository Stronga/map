...
public class SeattleTour : MonoBehaviour
{
    private static readonly List<MapScene> MapScenes =
        new List<MapScene>
        {
            ...
            new MapSceneOfLocationAndZoomLevel(new LatLon(39.32554, -82.10146), 17.0f),
            // MOHAI/wooden boats
            new MapSceneOfLocationAndZoomLevel(new LatLon(39.32748, -82.10027), 17.0f),
            new MapSceneOfLocationAndZoomLevel(new LatLon(39.32734, -82.10107), 18.0f),
            new MapSceneOfLocationAndZoomLevel(new LatLon(39.32473, -82.10232), 19.0f),
            // Gas Works
    ...
    // Public method to set the scene, to be called by button click
    public void SetScene(int index)
    {
        if (index >= 0 && index < MapScenes.Count)
        {
            StartCoroutine(SetMapScene(MapScenes[index]));
        }
    }

    private IEnumerator SetMapScene(MapScene scene)
    {
        yield return _map.SetMapScene(scene);
        yield return _map.WaitForLoad();
    }
